...
var remoteAccess = builder.Configuration.GetValue("WebUi:RemoteAccess", false);
if (!hasExplicitUrls)
{
    var webUiPort = builder.Configuration.GetValue("WebUi:Port", 5074);
    var webUiHost = remoteAccess
        ? "0.0.0.0"
        : builder.Configuration.GetValue("WebUi:Host", "127.0.0.1");
...
if (remoteAccess)
{
    ...
    var configuredToken = settingsService.Settings.Advanced.WebUiRemoteToken;
    if (string.IsNullOrWhiteSpace(configuredToken))
        configuredToken = builder.Configuration["WebUi:AuthToken"];
    if (string.IsNullOrWhiteSpace(configuredToken))
        throw new InvalidOperationException("WebUI remote access requires an explicit authentication token.");
}
