...
public class WebUiAutostartService(SettingsService settingsService, ILogger<WebUiAutostartService> logger)
{
    ...
    private void EnsureRemoteTokenConfigured()
    {
        settingsService.Load();
        if (string.IsNullOrWhiteSpace(settingsService.Settings.Advanced.WebUiRemoteToken))
            throw new InvalidOperationException("开启 WebUI 远程访问前必须先设置访问令牌。");
    }
