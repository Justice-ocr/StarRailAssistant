...
class RuntimeSession:
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            if is_session_active() or self._is_lock_owner_alive():
                return False
            return self._replace_stale_lock(payload)
        except Exception:
            return False

    @staticmethod
    def _is_lock_owner_alive() -> bool:
        try:
            lock_data = json.loads(LockFile.read_text(encoding="utf-8"))
            return _is_process_alive(int(lock_data.get("pid", 0)))
