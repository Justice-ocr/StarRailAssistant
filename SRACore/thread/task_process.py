...
class TaskManager(Runner):
    ...
    def run(self, *args: str) -> bool:
        config_list = list(args) if args else load_cache().get("ConfigNames", [])
        session = RuntimeSession(owner="sra-cli", mode="run", config_names=[str(x) for x in config_list])
        ...
        try:
            ...
            session.finish("completed" if result else ("stopped" if self.stop_event.is_set() else "failed"))
            return result
        except Exception:
            session.finish("stopped" if self.stop_event.is_set() else "failed")
            raise
        finally:
            self._stop_runtime_watcher()

    def _run_impl(self, *args: str) -> bool:
        """
        进程主循环：
        1. 读取配置列表（单配置或多配置）
        2. 对每个配置加载任务列表并执行
