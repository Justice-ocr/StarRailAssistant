...
def _find_top_bar_jade(results: list[Any] | None) -> int | None:
...
class WarpForecastExtension(BaseExtension[WarpForecastConfig]):
    ...
    def _scan_passes(self) -> tuple[int | None, int | None]:
        ...
        if special_pass is None or normal_pass is None:
            raise RuntimeError("未完整识别专票和通票数量，请核对背包或改用手动资源配置")
    ...
    def _open_reward_guide(self) -> bool:
        ...
        logger.warning('已尝试点击奖励指南，但未识别到"剩余"文本')
        return False
